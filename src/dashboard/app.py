...
def _render_control_panel() -> None:
    ...
    with st.form("runtime-config-form"):
        ...
        auto_analysis_enabled = st.checkbox("autoAnalysisEnabled", value=bool(runtime_config.get("autoAnalysisEnabled", False)))
        ...
        ema_short = st.number_input("emaShort", min_value=1, step=1, value=int(runtime_config.get("emaShort", 5)))
        ema_mid = st.number_input("emaMid", min_value=1, step=1, value=int(runtime_config.get("emaMid", 35)))
        use_volume_filter = st.checkbox("useVolumeFilter", value=bool(runtime_config.get("useVolumeFilter", False)))
        volume_multiplier = st.number_input("volumeMultiplier", min_value=0.0, step=0.1, value=float(runtime_config.get("volumeMultiplier", 1.05)))
        use_adx_filter = st.checkbox("useAdxFilter", value=bool(runtime_config.get("useAdxFilter", False)))
        adx_len = st.number_input("adxLen", min_value=1, step=1, value=int(runtime_config.get("adxLen", 14)))
        adx_threshold = st.number_input("adxThreshold", min_value=0.0, step=0.5, value=float(runtime_config.get("adxThreshold", 14.0)))
        cooldown_candles = st.number_input("cooldownCandles", min_value=0, step=1, value=int(runtime_config.get("cooldownCandles", 1)))
